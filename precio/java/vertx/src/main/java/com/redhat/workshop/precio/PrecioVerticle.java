...
public class PrecioVerticle extends AbstractVerticle {
    ...
    private static final String RESPONSE_STRING_FORMAT = "precio v1 desde '%s': %d\n";
    ...
    private void getPrecioData(RoutingContext ctx) {
        if (misbehave) {
            ...
            ctx.response().setStatusCode(503).end(String.format("Recomendación misbehavior desde '%s'\n", HOSTNAME));
        } else {
            count++;
